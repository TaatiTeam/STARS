...
class IO():
    ...
    def load_weights(self, model, weights_path, ignore_weights=None, rename_weights=None, output=False):
        ...
        for r in rename_weights:
            ...
            if i == '_':
                for w in list(weights.keys()):
                    weights[j + '.' + w] = weights.pop(w)
                    if output:
                        self.print_log('Add [{}] to [{}].'.format(j, w))
            elif j == '_':
                for w in list(weights.keys()):
                    weights[w.split(i + '.')[-1]] = weights.pop(w)
                    if output:
                        self.print_log('Del [{}] from [{}].'.format(i, w))
            else:

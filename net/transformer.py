...
class SkeleEmbed(nn.Module):
    ...
    def __init__(
        ...
        self.proj = nn.Conv2d(
            dim_in, dim_feat, kernel_size=kernel_size, stride=kernel_size)
...
class FCHead(nn.Module):
    def __init__(self, dim_feat, num_class):
        super().__init__()

        self.fc = nn.Linear(dim_feat, num_class)

    def forward(self, x, N, M, TP, VP):
        x_reshaped = x.reshape(N, M, TP * VP + 1, -1)
        x_pooled = x_reshaped.mean(dim=1)

        x_cls = self.fc(x_pooled[:, 0])

        return x_cls
    

class PretrainHead(nn.Module):
    def __init__(self, dim_feat, predictor_hidden_dim, num_class):
        super().__init__()
        ...
        self.predictor = nn.Sequential(
            nn.Linear(dim_feat, predictor_hidden_dim, bias=False),
            nn.BatchNorm1d(predictor_hidden_dim),
            nn.ReLU(),
            nn.Linear(predictor_hidden_dim, num_class),
        )
    ...
    def forward(self, x, N, M, TP, VP):
        ...
        x_project = self.projector(x)
        x_predict = self.predictor(x_project)

        return x_predict, x_project
    

class Transformer(nn.Module):
    def __init__(self, dim_in=3, num_class=60, dim_feat=256,
                 depth=4, num_heads=8, mlp_ratio=4,
                 num_frames=120, num_joints=25, patch_size=1, t_patch_size=4,
                 qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0., mask_ratio=0.0,
                 motion_aware_tau=0.75, drop_path_rate=0., clr_pretrain=False,
                 predictor_hidden_dim=4096, norm_layer=nn.LayerNorm, cls_token=False, **kwargs):
        super().__init__()
        ...
        self.clr_pretrain = clr_pretrain
        if self.mask_ratio > 0:
            self.masked_embed = nn.Parameter(torch.zeros(1, 1, dim_feat))
        if cls_token:
        ...
        self.joints_embed = SkeleEmbed(
            dim_in, dim_feat, num_frames, num_joints, patch_size, t_patch_size)
        ...
        if cls_token:
            self.head = FCHead(dim_feat, num_class)
        else:
            if clr_pretrain:
                self.head = PretrainHead(dim_feat, predictor_hidden_dim, num_class)
            else:
                self.head = nn.Linear(dim_feat, num_class)
    ...
    def patchify(self, sequences):
        """
        sequences: (N, T, V, 3)
        x: (N, L, t_patch_size * patch_size * 3)
        """
        NM, T, V, C = sequences.shape
        p = self.patch_size
        u = self.t_patch_size
        ...
        x = sequences.reshape(shape=(NM, TP, u, VP, p, C))
    ...
    def forward(self, x, return_feature=False):
        ...
        if self.cls_token is not None:
            cls_token = self.cls_token.expand(NM, -1, -1)
        ...
        if self.cls_token is not None:
            x = self.head(x, N, M, TP, VP)
        else:
            if self.clr_pretrain:
                x_predict, x_project = self.head(x, N, M, TP, VP)
                if return_feature: 
                    return x_project
            else:
                x = x.reshape(N, M, TP * VP, -1)
...
def _test():
    model = Transformer(cls_token=True)
    input_sequence = torch.randn(4, 3, 120, 25, 2)
    print(model(input_sequence).shape)
